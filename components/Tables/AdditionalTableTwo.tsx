...
const AdditionalTableTwo: React.FC<AdditionalTableProps> = ({ tableData, unrealisedGainLoss, transactionID, additionalData }) => {
    ...
    <div className="bg-white pb-2.5 dark:border-strokedark dark:bg-boxdark xl:pb-1">
      ...
      {filteredTableData.map((data, index) => (
        ...
        >
          ...
          <div className="flex items-center justify-center p-2.5 xl:p-5">
            <p className={`${data.GainLoss < 0 ? "text-meta-1" : "text-meta-3"}`}>{((data.GainLoss / data.TotalPaid) * 100).toFixed(2)}%</p>
          </div>

          <div className="flex items-center space-x-3.5">
            <button style={{ visibility: "hidden", width: "18px", height: "18px", padding: "0", margin: "0" }}></button>

            <button onClick={() => openDeleteModal(data.TransactionID)} className="hover:text-meta-1">
